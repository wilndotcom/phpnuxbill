...
function mpesa_c2b_validate_config()
{
    global $config;
...
function mpesa_c2b_show_config()
{
    global $ui;
    $ui->assign('_title', 'M-Pesa C2B - Payment Gateway');
    $ui->display('paymentgateway/ui/payment.tpl');
}

function mpesa_c2b_save_config()
{
    global $admin, $_L;
    ...
    r2(U . 'paymentgateway/mpesa_c2b', 's', Lang::T('Settings_Saved_Successfully'));
}

function mpesa_c2b_create_transaction($trx, $user)
{
    global $config;

    // Get access token
    $access_token = mpesa_c2b_get_access_token();
    if (!$access_token) {
    ...
    $callback_url = U . 'callback/mpesa_c2b';
    ...
    $result = mpesa_c2b_stk_push($stk_request, $access_token);
...
function mpesa_c2b_payment_notification()
{
...
function mpesa_c2b_get_status($trx, $user)
{
...
function mpesa_c2b_get_access_token()
{
    global $config;

    $url = mpesa_c2b_get_base_url() . '/oauth/v1/generate?grant_type=client_credentials';
...
function mpesa_c2b_stk_push($data, $access_token)
{
    $url = mpesa_c2b_get_base_url() . '/mpesa/stkpush/v1/processrequest';
...
function mpesa_c2b_get_base_url()
{
    global $config;
